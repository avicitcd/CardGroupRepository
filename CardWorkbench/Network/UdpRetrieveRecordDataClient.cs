...
namespace CardWorkbench.Network
{
    ...
    public class UdpRetrieveRecordDataClient
    {
        ...
        public static bool isMulticast = false;
        // 异步状态同步
        ...
        DynamicBindingList row_lst = new DynamicBindingList();  //grid数据List
        private GridControl frameDataGrid = null;   //原始帧grid对象
        private int frameSize = 0;  //帧长
        private int grid_frame_num = 0; //帧深
        private int wordByteCount = 0;   //字长(字节)
        ...
        public void frameDumpReceiveData(GridControl frameDataGrid, int frameSize, int wordSize, int grid_frame_num) 
        {
            ...
            this.wordByteCount = wordSize / 8;
            this.grid_frame_num = grid_frame_num;
        ...
        private void doWork(object sender, EventArgs e)
        {
            ...
            //1.开始接收数据的任务
            //if (frameDumpReceiveBytes == null || frameDumpReceiveBytes.Length == 0)
            //{
                if (cancellationTokenSource.Token.IsCancellationRequested)
                {
                    return;
                }
                        if (writer == null || recordFs == null) //如果同时开启记录模块，则每次获取的数据由记录模块提供
                        {
            ...
            //2. 开始界面取数据显示
            if (row_lst.Count == this.grid_frame_num)
            {
            ...
            using (MemoryStream stream = new MemoryStream(frameDumpReceiveBytes))
                {
                    ...
                    int numBytesToRead = frameDumpReceiveBytes.Length;
                    DynamicDictionary frameDumpDisplayDic = new DynamicDictionary();  //子帧数据dictionary

                    while (numBytesToRead >= 0)
                    {
                        bytes = new byte[this.wordByteCount];
                        if (readCount > this.frameSize)   //读取的数据满了一帧，则又从头开始
                        {
                            lossByteNum = 0;
                            ...
                            //{
                                ...
                                if (row_lst.Count > this.grid_frame_num)    //达到指定帧深后从首行更新
                                {
                                    row_lst.RemoveAt(this.grid_frame_num);
                        ...
                        int n = stream.Read(bytes, 0, this.wordByteCount);
                        if (n == 0)
                        {
                            break;
                        }
                        numBytesToRead -= n;
                        //// start 每次开始接收一个新的子帧时，处理时间，状态
                        if (dataReceiver != null && (dataReceiver.bEnableTime == 1 || dataReceiver.bEnableStatus == 1) && readCount == 1)
                        {
                            if (dataReceiver.bEnableTime == 1 && dataReceiver.bEnableStatus == 1 && lossByteNum < 6)   //有时间有状态
                            {
                                lossByteNum += 1;
                                continue;
                            }
                            if (dataReceiver.bEnableTime == 1 && dataReceiver.bEnableStatus == 0 && lossByteNum < 4)  //有时间无状态
                            {
                                lossByteNum += 1;
                                continue;
                            }
                            if (dataReceiver.bEnableTime == 0 && dataReceiver.bEnableStatus == 1 && lossByteNum < 2)  //有状态无时间
                            {
                                lossByteNum += 1;
                                continue;
                            }
